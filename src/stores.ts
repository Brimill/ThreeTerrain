...
export const useAppStore = create<AppState>()((set) => ({
  ...
  setFrequencies: (newFrequencies: number[]) =>
    set({ frequencies: newFrequencies }),
  setFrequency: (index: number, value: number) =>
    set((state) => {
      const newFrequencies = [...state.frequencies];
      newFrequencies[index] = value;
      return { frequencies: newFrequencies };
    }),
  addFrequency: (newFrequency: number) =>
    set((state) => ({
  ...
  setAmplitudes: (newAmplitudes: number[]) =>
    set({
      amplitudes: newAmplitudes,
    }),
  setAmplitude: (index: number, value: number) =>
    set((state) => {
      const newAmplitudes = [...state.amplitudes];
      newAmplitudes[index] = value;
      return { amplitudes: newAmplitudes };
    }),
  addAmplitude: (newAmplitude: number) =>
    set((state) => ({
  ...
  setNoiseTextures: (newTextures: ImageData[]) =>
    set({ noiseTextures: newTextures }),
}));
